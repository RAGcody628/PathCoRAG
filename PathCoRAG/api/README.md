...
| --key | None | API key for authentication. Protects PathCoRAG server against unauthorized access |
...
#### Running a PathCoRAG server with ollama default local server as llm and embedding backends

Ollama is the default backend for both llm and embedding, so by default you can run PathCoRAG-server with no parameters and the default ones will be used. Make sure ollama is installed and is running and default models are already installed on ollama.

```bash
# Run PathCoRAG with ollama, mistral-nemo:latest for llm, and bge-m3:latest for embedding
PathCoRAG-server

# Using specific models (ensure they are installed in your ollama instance)
PathCoRAG-server --llm-model adrienbrault/nous-hermes2theta-llama3-8b:f16 --embedding-model nomic-embed-text --embedding-dim 1024

# Using an authentication key
PathCoRAG-server --key my-key

# Using lollms for llm and ollama for embedding
PathCoRAG-server --llm-binding lollms
```

#### Running a PathCoRAG server with lollms default local server as llm and embedding backends
