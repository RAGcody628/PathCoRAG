...
class OllamaServerInfos:
    ...
    PathCoRAG_NAME = "PathCoRAG"
    PathCoRAG_TAG = os.getenv("OLLAMA_EMULATING_MODEL_TAG", "latest")
    PathCoRAG_MODEL = f"{PathCoRAG_NAME}:{PathCoRAG_TAG}"
    PathCoRAG_SIZE = 7365960935  # it's a dummy value
    PathCoRAG_CREATED_AT = "2024-01-15T00:00:00Z"
    PathCoRAG_DIGEST = "sha256:PathCoRAG"


ollama_server_infos = OllamaServerInfos()


# query mode according to query prefix (bypass is not PathCoRAG quer mode)
...
class OllamaAPI:
    def __init__(self, rag: PathCoRAG):
        self.rag = rag
    ...
    def setup_routes(self):
        ...
        async def get_tags():
            ...
            return OllamaTagResponse(
                models=[
                    {
                        "name": self.ollama_server_infos.PathCoRAG_MODEL,
                        "model": self.ollama_server_infos.PathCoRAG_MODEL,
                        "size": self.ollama_server_infos.PathCoRAG_SIZE,
                        "digest": self.ollama_server_infos.PathCoRAG_DIGEST,
                        "modified_at": self.ollama_server_infos.PathCoRAG_CREATED_AT,
                        "details": {
                            "parent_model": "",
                            "format": "gguf",
                            "family": self.ollama_server_infos.PathCoRAG_NAME,
                            "families": [self.ollama_server_infos.PathCoRAG_NAME],
                            "parameter_size": "13B",
        ...
        async def generate(raw_request: Request, request: OllamaGenerateRequest):
            ...
            For compatibility purpose, the request is not processed by PathCoRAG,
                    ...
                    async def stream_generator():
                        try:
                            ...
                            if isinstance(response, str):
                                ...
                                data = {
                                    "model": self.ollama_server_infos.PathCoRAG_MODEL,
                                    "created_at": self.ollama_server_infos.PathCoRAG_CREATED_AT,
                                    "response": response,
                                    "done": False,
                                }
        ...
        async def chat(raw_request: Request, request: OllamaChatRequest):
            ...
            Routes user queries through PathCoRAG by selecting query mode based on prefix indicators.
