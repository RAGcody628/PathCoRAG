...
from PathCoRAG.types import GPTKeywordExtractionFormat
from PathCoRAG.api import __api_version__
...
mongo_database = config.get("mongodb", "database", fallback="PathCoRAG")
...
def display_splash_screen(args: argparse.Namespace) -> None:
    """
    Display a colorful splash screen showing PathCoRAG server configuration
    ...
    ASCIIColors.cyan(f"""
    ╔══════════════════════════════════════════════════════════════╗
    ║                   🚀 PathCoRAG Server v{__api_version__}                  ║
    ...
    ASCIIColors.yellow(f"{ollama_server_infos.PathCoRAG_MODEL}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="PathCoRAG FastAPI Server with separate working and input directories"
    )
    ...
    parser.add_argument(
        "--key",
        type=str,
        default=get_env_value("PathCoRAG_API_KEY", None),
        help="API key for authentication. This protects PathCoRAG server against unauthorized access",
    )
    ...
    parser.add_argument(
        ...
        default=get_env_value(
            "SIMULATED_MODEL_NAME", ollama_server_infos.PathCoRAG_MODEL
        ),
        help="Number of conversation history turns to include (default: from env or 3)",
    )
    ...
    ollama_server_infos.PathCoRAG_MODEL = args.simulated_model_name
...
def create_app(args):
    ...
    api_key = os.getenv("PathCoRAG_API_KEY") or args.key
    ...
    app = FastAPI(
        title="PathCoRAG API",
        description="API for querying text using PathCoRAG with separate storage and input directories"
        + "(With authentication)"
        if api_key
        else "",
    ...
    if args.llm_binding == "lollms" or args.embedding_binding == "lollms":
        from PathCoRAG.llm.lollms import lollms_model_complete, lollms_embed
    ...
    if args.llm_binding == "azure_openai" or args.embedding_binding == "azure_openai":
        from PathCoRAG.llm.azure_openai import (
            azure_openai_complete_if_cache,
            azure_openai_embed,
        )
    if args.llm_binding_host == "openai-ollama" or args.embedding_binding == "ollama":
        from PathCoRAG.llm.openai import openai_complete_if_cache
    ...
    async def clear_documents():
        """
        Clear all documents from the PathCoRAG system.

        This endpoint deletes all text chunks, entities vector database, and relationships vector database,
        effectively clearing all documents from the PathCoRAG system.
