...
class PGDocStatusStorage(DocStatusStorage):
    ...
    async def filter_keys(self, data: set[str]) -> set[str]:
        ...
        sql = (
            f"SELECT id FROM PathCoRAG_DOC_STATUS WHERE workspace=$1 AND id IN ({keys})"
        )
    ...
    async def get_by_id(self, id: str) -> Union[dict[str, Any], None]:
        sql = "select * from PathCoRAG_DOC_STATUS where workspace=$1 and id=$2"
        params = {"workspace": self.db.workspace, "id": id}
    ...
    async def get_docs_by_status(
        ...
        sql = "select * from PathCoRAG_DOC_STATUS where workspace=$1 and status=$2"
        params = {"workspace": self.db.workspace, "status": status}
        result = await self.db.query(sql, params, True)
        return {
    ...
    async def upsert(self, data: dict[str, dict]):
        ...
        """
        sql = """insert into PathCoRAG_DOC_STATUS(workspace,id,content,content_summary,content_length,chunks_count,status)
...
NAMESPACE_TABLE_MAP = {
    ...
    NameSpace.VECTOR_STORE_ENTITIES: "PathCoRAG_VDB_ENTITY",
    NameSpace.VECTOR_STORE_RELATIONSHIPS: "PathCoRAG_VDB_RELATION",
    NameSpace.DOC_STATUS: "PathCoRAG_DOC_STATUS",
    NameSpace.KV_STORE_LLM_RESPONSE_CACHE: "PathCoRAG_LLM_CACHE",
}
...
TABLES = {
    "PathCoRAG_DOC_FULL": {
        "ddl": """CREATE TABLE PathCoRAG_DOC_FULL (
	                ...
	                CONSTRAINT PathCoRAG_DOC_FULL_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "PathCoRAG_DOC_CHUNKS": {
        "ddl": """CREATE TABLE PathCoRAG_DOC_CHUNKS (
                    ...
                    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP,
	                CONSTRAINT PathCoRAG_DOC_CHUNKS_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "PathCoRAG_VDB_ENTITY": {
        "ddl": """CREATE TABLE PathCoRAG_VDB_ENTITY (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    ...
                    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP,
	                CONSTRAINT PathCoRAG_VDB_ENTITY_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "PathCoRAG_VDB_RELATION": {
        "ddl": """CREATE TABLE PathCoRAG_VDB_RELATION (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    ...
                    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP,
	                CONSTRAINT PathCoRAG_VDB_RELATION_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "PathCoRAG_LLM_CACHE": {
        "ddl": """CREATE TABLE PathCoRAG_LLM_CACHE (
                    ...
                    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP,
	                CONSTRAINT PathCoRAG_LLM_CACHE_PK PRIMARY KEY (workspace, mode, id)
                    )"""
    },
    "PathCoRAG_DOC_STATUS": {
        "ddl": """CREATE TABLE PathCoRAG_DOC_STATUS (
	               workspace varchar(255) NOT NULL,
	               id varchar(255) NOT NULL,
                   content TEXT,
	               ...
	               CONSTRAINT PathCoRAG_DOC_STATUS_PK PRIMARY KEY (workspace, id)
	              )"""
    },
}
...
SQL_TEMPLATES = {
    ...
    "get_by_id_full_docs": """SELECT id, COALESCE(content, '') as content
                                FROM PathCoRAG_DOC_FULL WHERE workspace=$1 AND id=$2
                            """,
    "get_by_id_text_chunks": """SELECT id, tokens, COALESCE(content, '') as content,
                                chunk_order_index, full_doc_id
                                FROM PathCoRAG_DOC_CHUNKS WHERE workspace=$1 AND id=$2
                            """,
    "get_by_id_llm_response_cache": """SELECT id, original_prompt, COALESCE(return_value, '') as "return", mode
                                FROM PathCoRAG_LLM_CACHE WHERE workspace=$1 AND mode=$2
                               """,
    "get_by_mode_id_llm_response_cache": """SELECT id, original_prompt, COALESCE(return_value, '') as "return", mode
                           FROM PathCoRAG_LLM_CACHE WHERE workspace=$1 AND mode=$2 AND id=$3
                          """,
    "get_by_ids_full_docs": """SELECT id, COALESCE(content, '') as content
                                 FROM PathCoRAG_DOC_FULL WHERE workspace=$1 AND id IN ({ids})
                            """,
    "get_by_ids_text_chunks": """SELECT id, tokens, COALESCE(content, '') as content,
                                  chunk_order_index, full_doc_id
                                   FROM PathCoRAG_DOC_CHUNKS WHERE workspace=$1 AND id IN ({ids})
                                """,
    "get_by_ids_llm_response_cache": """SELECT id, original_prompt, COALESCE(return_value, '') as "return", mode
                                 FROM PathCoRAG_LLM_CACHE WHERE workspace=$1 AND mode= IN ({ids})
                                """,
    "filter_keys": "SELECT id FROM {table_name} WHERE workspace=$1 AND id IN ({ids})",
    "upsert_doc_full": """INSERT INTO PathCoRAG_DOC_FULL (id, content, workspace)
                        VALUES ($1, $2, $3)
                        ON CONFLICT (workspace,id) DO UPDATE
                           SET content = $2, update_time = CURRENT_TIMESTAMP
                       """,
    "upsert_llm_response_cache": """INSERT INTO PathCoRAG_LLM_CACHE(workspace,id,original_prompt,return_value,mode)
                                      VALUES ($1, $2, $3, $4, $5)
                                     ...
                                     """,
    "upsert_chunk": """INSERT INTO PathCoRAG_DOC_CHUNKS (workspace, id, tokens,
                      ...
                      update_time = CURRENT_TIMESTAMP
                     """,
    "upsert_entity": """INSERT INTO PathCoRAG_VDB_ENTITY (workspace, id, entity_name, content, content_vector)
                      VALUES ($1, $2, $3, $4, $5)
                      ON CONFLICT (workspace,id) DO UPDATE
                      ...
                      update_time=CURRENT_TIMESTAMP
                     """,
    "upsert_relationship": """INSERT INTO PathCoRAG_VDB_RELATION (workspace, id, source_id,
    ...
    "entities": """SELECT entity_name FROM
        ...
        FROM PathCoRAG_VDB_ENTITY where workspace=$1)
    ...
    "relationships": """SELECT source_id as src_id, target_id as tgt_id FROM
        ...
        FROM PathCoRAG_VDB_RELATION where workspace=$1)
        WHERE distance>$2 ORDER BY distance DESC  LIMIT $3
       """,
    "chunks": """SELECT id FROM
        (SELECT id, 1 - (content_vector <=> '[{embedding_string}]'::vector) as distance
        FROM PathCoRAG_DOC_CHUNKS where workspace=$1)
        WHERE distance>$2 ORDER BY distance DESC  LIMIT $3
       """,
}
