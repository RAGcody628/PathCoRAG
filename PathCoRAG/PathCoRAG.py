...
@dataclass
class PathCoRAG:
    """PathCoRAG: Simple and Fast Retrieval-Augmented Generation."""

    working_dir: str = field(
        default_factory=lambda: f'./PathCoRAG_cache_{datetime.now().strftime("%Y-%m-%d-%H:%M:%S")}'
    )
    ...
    def __post_init__(self):
        ...
        log_file = os.path.join(self.log_dir, "PathCoRAG.log")
        set_logger(log_file)
        ...
        logger.debug(f"PathCoRAG init with param:\n  {_print_config}\n")
