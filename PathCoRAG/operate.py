...
async def ours_kg_query(
    ...
    # 폴더 자동 생성
    os.makedirs("saved_contexts", exist_ok=True)

    # 타임스탬프 추가 (선택적)
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # 파일 열기 - append 모드 ("a")
    with open("saved_contexts/context_log.txt", "a", encoding="utf-8") as f:
        f.write("\n" + "=" * 80 + "\n")
        f.write(f"[{timestamp}] Query: {query}\n")
        f.write(f"High-level steps: {hl_extended_queries}\n")
        f.write(f"Low-level queries: {ll_extended_queries}\n")
